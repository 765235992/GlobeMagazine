...
- (void)commonInit {
    ...
    // 触发懒加载
    self.segmentView.dk_backgroundColorPicker = DKColorPickerWithKey(频道切换背景颜色);
...
- (JHContentView *)contentView {
    if (!_contentView) {
        JHContentView *content = [[JHContentView alloc] initWithFrame:CGRectMake(0.0, CGRectGetMaxY(self.segmentView.frame), self.bounds.size.width, self.bounds.size.height - CGRectGetMaxY(self.segmentView.frame)) childVcs:self.childVcs segmentView:self.segmentView parentViewController:self.parentViewController];
